...
echo "Extracting backup zip into /opt/hytale (expects universe/ inside zip)..."
unzip -o "$zip" -d /opt/hytale

if [ ! -d /opt/hytale/universe ]; then
  echo "Restore completed but /opt/hytale/universe was not created."
  echo "This likely means the backup zip doesn't contain 'universe/'. Inspect with: unzip -l \"$zip\" | head"
  exit 1
fi
